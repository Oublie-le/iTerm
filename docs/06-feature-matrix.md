...
| SSH Agent、密钥、密码交互和 PTY | 🟡 | 参数构造单测；跨平台 PTY 回环 | OpenSSH 服务端和 known_hosts 策略冒烟 |
| ADB 发现、授权状态和交互 Shell | 🟡 | 设备输出解析单测；跨平台 PTY 回环 | USB 与网络 ADB 真机验证 |
| ANSI、Unicode 11、WebGL、搜索和复位 | ✅ | 终端单测；百万行无渲染性能基线；E2E | WebGL/Canvas GUI FPS 与输入延迟基线 |
...
| WindTerm 视觉对照与 Golden 差分 | 🟡 | 核心工作区交互 E2E | 冻结参考版本、基线截图和差分阈值 |
