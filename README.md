...
- 多个发送器、循环发送和发送间隔；
- 每会话发送器模板持久化；
- 每会话命令历史、使用次数与快捷命令联想，支持 `⌘K` / `Ctrl+Shift+K` 调出；
- 发送器命令模板的版本化 JSON 导入/导出；
- 原始文件分块发送、进度显示和取消；
- 原始字节/文本日志及开始、暂停、继续、停止、大小限制和自动轮转；
- 一键打开当前日志文件或应用日志目录；
- Apple 风格的浅色、深色和跟随系统界面材料与终端主题；
- 核心工作区快捷键和应用内快捷键帮助；
- 终端选区复制、剪贴板粘贴、全选和右键操作菜单，支持平台常用快捷键；
- `Ctrl/⌘ +/-/0`、工具条及 `Ctrl/⌘ + 滚轮` 字号缩放，并按会话持久化；
...
- 会话配置、发送器、标签和工作区通过带迁移的本地 SQLite 数据库持久化；
- 全部会话配置的版本化 JSON 导入/导出和重复 ID 安全处理；
- 有界本地结构化诊断记录与隐私脱敏 JSON 导出；
- 一键清除 SQLite 与 WebView Storage 中的本地应用数据并恢复默认设置；
...
也可以手动填写主机、端口和用户名。启用严格主机密钥校验时，目标必须已存在于
...
ZModem 功能使用 Apache-2.0 许可的 `zmodem.js`，详情见
